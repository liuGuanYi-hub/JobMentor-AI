...
- 熟悉 MVVM / MVI / Clean Architecture`;
  ...
  const resume = `王小明（示例）
2027 届软件工程本科 | 专业前 5% | 移动端开发 · 后端工程实习 · AI 应用探索
具备移动端客户端与后端服务全链路开发经验，熟练掌握 Kotlin/Jetpack Compose 与 Java/Spring Boot，主导过完整业务模块从 0 到 1 的客户端实现；在校期间深耕工程化能力，关注性能优化、可测试性与跨端协作；对 AI/Agent 工程有持续探索，主导过 Agent 多轮对话引擎落地。

【教育经历】
某大学 · 软件工程专业 · 本科（2023.09 - 2027.06）
- 学业表现：GPA 3.8/4.0，专业排名前 5%，获国家奖学金（2024、2025 连续两年）
- 主修课程：数据结构与算法、操作系统、计算机网络、数据库系统、移动应用开发、软件工程、编译原理
- 校园角色：班级学习委员（组织期末复习串讲 6 次，覆盖 40+ 人）、校 ACM 程序设计竞赛集训队成员

【核心优势】
- 移动端工程：独立完成登录、健康档案、数据看板、服务预约等 6 大模块，覆盖全流程 90% 交互；通过索引与分页缓存将列表滑动帧率从 45fps 提升至 60fps
- 跨端与可测试性：基于 Repository 分层统一远程接口与本地数据访问，核心模块测试覆盖率从 0 提升至 78%；远程优先 + 本地回退策略让弱网可用性从 0% 提升至 95%
- 后端工程实践：实习期间重构数据聚合模块，日处理量从 50 万条提升至 200 万条（+300%），查询 P95 延迟从 800ms 降至 300ms
- AI 应用探索：主导校园服务助手 Agent 多轮对话引擎与事件触发链路，标注 500+ 条对话数据迭代提示词，意图识别精确率提升 12pt（86% → 98%）

【实习经历】
某互联网科技公司 · 后端工程师实习生（数据服务组）
时间：2026.04 - 2026.07
- 参与数据聚合服务与数据处理管道开发维护，负责订单流水与用户行为数据的清洗与聚合，输出供下游 BI/算法团队使用
- 用 Java 重构 ETL 流程，将日处理量从 50 万条提升至 200 万条，数据吞吐提升 30%，任务失败率下降 25%
- 通过索引优化与批次写入降低数据库压力，查询 P95 延迟从 800ms 降至 300ms；编写数据接口文档并参与 Code Review
- 沉淀团队统一异常处理规范，统一 5 类常见异常码映射，降低新人接入成本与跨团队沟通成本

【项目经历】
健康管理 App · 移动端客户端研发（课程项目 / 团队 4 人 · 担任客户端负责人）
时间：2024.10 - 2026.06
- 独立完成客户端核心功能模块，基于响应式 UI 实现登录、健康档案、数据看板、服务预约等 6 大模块，覆盖全流程 90% 交互
- 设计 Repository 分层统一远程接口与本地数据访问，支持单元测试中替换数据源，核心模块测试覆盖率提升至 78%
- 实现远程优先、本地回退的离线降级策略，弱网环境下页面可用性从 0% 提升到 95%，并区分数据来源状态避免展示过期数据
- 优化健康数据查询与列表加载，通过索引与分页缓存将列表滑动帧率从 45fps 提升至 60fps
- 协助后端完成接口联调，开发调试与性能监控工具，将联调问题定位时间缩短约 40%

校园服务助手 Agent · AI 应用开发（校内创新项目 / 团队 3 人）
时间：2026.02 - 2026.06
- 基于 Agent 框架集成多智能体，设计可热更新的调度策略，支持 8 类校园事务技能按需加载
- 构建数据驱动评估体系，通过标注 500+ 条对话数据迭代提示词，意图识别精确率提升 12pt（86% → 98%）
- 面向请假、证明材料、奖助学金、活动通知等高频场景搭建对话引擎，支持多轮对话与任务状态联动
- 通过 Webhook/WebSocket 接入协同平台，设计事件触发链路，日处理消息峰值 2000+ 条

校内二手交易小程序 · 全栈开发（个人项目 / 独立完成）
时间：2025.09 - 2026.01
- 基于 Kotlin + Jetpack Compose + Spring Boot + MySQL 全栈开发，前后端独立完成
- 设计基于角色的权限模型（学生 / 认证商家 / 管理员），覆盖发布、搜索、聊天、订单全流程 5 大模块
- 实现基于关键字 + 地理索引的混合搜索，单关键词查询 P95 < 200ms，覆盖 300+ 商品
- 完成 12pt 字号无障碍适配与暗色模式适配；应用启动时间从 2.1s 优化至 1.2s（启动器 + 懒加载 + 任务编排）

【技术能力】
- 移动端：Kotlin（协程 / Flow）、Jetpack Compose、MVVM、MVI、Repository、Room、Retrofit、Hilt
- 后端 & 数据：Java、Spring Boot、MySQL（索引 / 事务 / 锁）、Redis、RabbitMQ、MyBatis、ETL 流程
- AI 应用：LangGraph、Agent 框架集成、意图识别与槽位抽取、RAG 检索、Webhook / WebSocket 异步事件
- 工程化 & 工具链：Git、Linux、Docker、Postman、JUnit5、CI/CD（GitHub Actions）、性能监控与日志分析
- 语言 & 软技能：英语 CET-6、文档协作（Notion / Markdown）、内部分享（3 次主讲，覆盖 60+ 人次）

【竞赛与荣誉】
- 校级 ACM 程序设计竞赛二等奖（2025）
- 国家励志奖学金（2024、2025 连续两年）
- 校级"互联网+"创新创业大赛三等奖（2025，主导 MVP 演示）`;
